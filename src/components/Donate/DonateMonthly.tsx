...
export default function DonateMonthly() {
  ...
  const limitCaution =
    leavecommentMessage.length > 520 && leavecommentMessage.length < 600;

  const { isLoading, ...createCheckout } = trpc.useMutation(
    "checkout.create-monthly-donation"
  );
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      "leavecommentMessage",
      JSON.stringify(leavecommentMessage)
    );
      ...
      <form onSubmit={handleDonationSubmit}>
        <div className="grid grid-cols-3 gap-4 mb-4">
          ...
          <p className="col-span-3">
            Your donation of{" "}
            <span className="font-bold">{selectedOption.priceLabel}</span>{" "}
        ...
        <div className="space-y-4">
          <div className="inline-flex flex-col">
            <Checkbox
              ...
              onChange={() =>
                setInHonorChecked((inhonorChecked) => !inhonorChecked)
              }
            />
            <Checkbox
              ...
              onChange={() =>
                setLeaveCommentChecked(
                  (leavecommentChecked) => !leavecommentChecked
                )
              }
            />
          </div>
          ...
          >
            {inhonorChecked && (
              <div className="flex items-center justify-between">
                <Select
                  options={honorandmemoryOptions}
                  state={updateOnBehalfOf}
                />
                <UncontrolledInput
                  variant="text"
                  name="inHonorOf"
