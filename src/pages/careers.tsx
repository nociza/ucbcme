  ...
  return (
    <div>
      ...
      <section>
        ...
        >
          Engage with a purpose driven team or enjoy working solo. Work when
          you&apos;re inspired or rest when you&apos;re not. We believe the
          ideal work environment comes from putting the mental health of our
          team first.
        </HeaderSection>
      </section>
      ...
      <section className="bg-black text-white rounded-3xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-10 sm:grid-cols-3">
          {careerData.map((career) => (
            <div key={career.id}>
              <div
                className={`rounded-xl border-4 p-4 text-left h-full ${career.border}`}
              >
                <div key={career.id}>
                  <Image src={career.icon} width={48} height={48} alt="" />
                </div>
                <div className="p-1">
                  <h4 className={`font-semibold text-base ${career.color}`}>
                    {career.id}
                  </h4>
